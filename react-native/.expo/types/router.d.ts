...
export namespace ExpoRouter {
  type StaticRoutes = `/` | `/(app)` | `/(auth)/sign-in` | `/(auth)/sign-in-real` | `/(auth)/sign-up` | `/(feed)` | `/(post)` | `/(profile)/` | `/(profile)/(profile_private)` | `/(profile)/(profile_self)` | `/(profile)/(profile_viewable)` | `/(profile_private)` | `/(profile_self)` | `/(profile_viewable)` | `/(user)` | `/_sitemap` | `/add_like` | `/modal` | `/seeAll` | `/sign-in` | `/sign-in-real` | `/sign-up`;
